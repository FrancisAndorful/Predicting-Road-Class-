    ...
    def __init__(self, csv_file, transform=None):
        ...
        with open(csv_file, 'r') as file:
            ...
            for row in reader:
                ...
                data = [1.0 if x else 0.0 for x in row[4:-2]]
                data = torch.tensor(data, dtype=torch.float)
                label = torch.tensor(label, dtype=torch.long)
                data_dis = [float(row[-2])]
                data_dis = torch.tensor(data_dis, dtype=torch.float)
                fres=row[-1]
                ...
                data_con = np.concatenate((data, data_dis, self.frequency_vector))
